"""Fast classical CV segmentation of bright sulfide-like inclusions."""
...
DEFAULT_OVERLAY_COLOR_RGB = (255, 0, 0)
...
def segment_sulfides(
    ...
    """Segment bright sulfide-like inclusions from a microscopy image.

    Args:
        image: HxWx3 RGB or BGR image.
        config: Settings mapping, YAML path, or None for the default YAML.

    Returns:
        Binary uint8 mask with values 0 for background and 255 for candidates.
    """
...
def _segment_sulfides_cv(
    image: np.ndarray,
    cfg: dict[str, Any],
) -> np.ndarray:
    """Segment sulfide-like areas with the fast Otsu-only CV baseline."""
    ...
    offset = float(cfg["thresholds"].get("otsu_offset", -5))
    threshold = np.clip(float(otsu_threshold) + offset, 0, 255)
...
class MobileSamRefiner:
    ...
    def refine(
        ...
        component_ids = _largest_component_ids(stats, labels_count, min_area, max_components)
        ...
        with self.torch.inference_mode():
            for label_id in component_ids:
                ...
                padded_box = _padded_component_box(
                    stats[label_id],
                    width,
                    height,
                    box_padding,
...
def make_overlay(
    ...
    """Overlay a semi-transparent mask on an image and return RGB uint8 output."""

    cfg = load_sulfide_config(config)
    image_rgb = _to_uint8_rgb(image, cfg["preprocessing"].get("input_color", "rgb"))
    overlay_cfg = cfg.get("overlay", {})
    color = np.asarray(overlay_cfg.get("color_rgb", DEFAULT_OVERLAY_COLOR_RGB), dtype=np.float32)
...
def _create_mobile_sam_refiner(cfg: dict[str, Any]) -> MobileSamRefiner | None:
    sam_cfg = cfg.get("sam", {})
    if not bool(sam_cfg.get("enabled", False)):
        return None

    checkpoint = str(sam_cfg.get("checkpoint", "")).strip()
    if not checkpoint or checkpoint.startswith("<"):
        warnings.warn("MobileSAM checkpoint is not configured; using CV mask only.")
        return None
    ...
    if device == "cuda":
        try:
            ...
            warnings.warn(f"Could not check CUDA availability: {exc}; using CV mask only.")
            return None
        if not torch.cuda.is_available():
            warnings.warn("CUDA is not available; using CV mask only.")
            return None

    try:
        return MobileSamRefiner(checkpoint=checkpoint, device=device)
    except Exception as exc:
        warnings.warn(f"Could not initialize MobileSAM refiner: {exc}; using CV mask only.")
        return None
...
def run_directory(
    ...
    cfg = load_sulfide_config(config)
    paths = _image_paths(input_path, image_glob or cfg.get("visualization", {}).get("image_glob", "*.JPG"))
    ...
    bgr_cfg = _with_input_color(cfg, "bgr")
    refiner = _create_mobile_sam_refiner(cfg)
    sam_cfg = cfg.get("sam", {})
    ...
    for image_path in paths:
        ...
        cv_mask = _segment_sulfides_cv(image_bgr, bgr_cfg)
        cv_ms = _elapsed_ms(cv_start)
        if refiner is None:
            refined_mask = cv_mask.copy()
            refine_timings = {"set_image_ms": 0.0, "prompts_ms": 0.0, "total_refine_ms": 0.0}
            refine_stats = {"components_considered": 0, "components_refined": 0, "components_fallback": 0}
        else:
            image_rgb = _to_uint8_rgb(image_bgr, "bgr")
            ...
            refine_timings = refiner.last_timings.copy()
            refine_stats = refiner.last_stats.copy()

        cv_overlay_rgb = make_overlay(image_bgr, cv_mask, bgr_cfg)
        refined_overlay_rgb = make_overlay(image_bgr, refined_mask, bgr_cfg)
        target_dir = output_root / image_path.stem
        target_dir.mkdir(parents=True, exist_ok=True)
        cv2.imwrite(str(target_dir / "cv_mask.png"), cv_mask)
        cv2.imwrite(str(target_dir / "refined_mask.png"), refined_mask)
        cv2.imwrite(str(target_dir / "cv_overlay.png"), cv2.cvtColor(cv_overlay_rgb, cv2.COLOR_RGB2BGR))
        cv2.imwrite(str(target_dir / "refined_overlay.png"), cv2.cvtColor(refined_overlay_rgb, cv2.COLOR_RGB2BGR))
        total_ms = _elapsed_ms(total_start)
        records.append(
            {
                "image": str(image_path),
                "output_dir": str(target_dir),
                "cv_mask_pixels": int(np.count_nonzero(cv_mask)),
                "refined_mask_pixels": int(np.count_nonzero(refined_mask)),
                "cv_ms": cv_ms,
                "set_image_ms": float(refine_timings.get("set_image_ms", 0.0)),
                "prompts_ms": float(refine_timings.get("prompts_ms", 0.0)),
                "total_ms": total_ms,
                **refine_stats,
            }
        )
...
def _image_paths(input_path: str | Path, image_glob: str) -> list[Path]:
    input_value = Path(input_path)
    if input_value.is_dir():
        return sorted(path for path in input_value.glob(image_glob) if path.is_file())
    return [input_value]


def _with_input_color(cfg: dict[str, Any], input_color: str) -> dict[str, Any]:
    copied = {
        key: value.copy() if isinstance(value, dict) else value
        for key, value in cfg.items()
    }
    preprocessing = copied.setdefault("preprocessing", {})
    preprocessing["input_color"] = input_color
    return copied


def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Segment bright sulfide-like inclusions with classical CV.",
    )
    parser.add_argument("--input", default="data/img", help="Input image or directory.")
    parser.add_argument("--config", default=str(DEFAULT_CONFIG_PATH), help="YAML config path.")
    parser.add_argument(
        "--output",
        default="test/outputs/sulfide_candidates",
        help="Output directory for masks and overlays.",
    )
    parser.add_argument("--image-glob", default=None, help="Glob used when --input is a directory.")
...
def main() -> None:
    ...
    for record in records:
        print(
            ...
            f"cv={record['cv_mask_pixels']} px, refined={record['refined_mask_pixels']} px, "
            f"cv={record['cv_ms']:.1f} ms, set_image={record['set_image_ms']:.1f} ms, "
            f"prompts={record['prompts_ms']:.1f} ms, total={record['total_ms']:.1f} ms"
        )
