...
class ServiceConfig:
    ...
    def model_status(self) -> dict[str, dict[str, object]]:
        def status(path: Path | None) -> dict[str, object]:
            if path is None:
                return {"status": "unavailable", "reason": "checkpoint_not_configured"}
            if not path.is_file():
                return {"status": "unavailable", "reason": "checkpoint_not_found"}
            return {"status": "configured", "reason": None}

        return {
            "talc": status(self.talc_checkpoint_path),
            "sulfide": status(self.sulfide_checkpoint_path),
        }
